...
function textures(file, paletteColors) {
    ...
    var texels = fs.readFileSync(file);
    var textureBuffer = texels.slice(b44);
    var outputBuffer = [];
    var i = Control.palette;
    for (var j = 0; j < textureBuffer.length; j++) {
        var color1 = paletteColors[(textureBuffer[j] & 0xF) + i];
        var red1 = 0xFF & (color1 >> c[1]);
        var blue1 = 0xFF & (color1 >> c[2]);
        var green1 = 0xFF & (color1 >> c[3]);
        var color2 = paletteColors[(textureBuffer[j] >> c[4] & 0xF) + i];
        var red2 = 0xFF & (color1 >> c[5]);
        var blue2 = 0xFF & (color1 >> c[6]);
        var green2 = 0xFF & (color1 >> c[7]);
    ...
    ctx.resetTransform();
    var data = ctx.createImageData(64, b1664);
    for (var i = 0; i < buffer.length; i += b3) {
        data.data[i / b3 * b4] = buffer[i];
        data.data[i / b3 * b4 + b1] = buffer[i + b1];
        data.data[i / b3 * b4 + b2] = buffer[i + b2];
        data.data[i / b3 * b4 + b3] = 0xff;
    }
    var base64s = [];
    for (var i = 0; i < buffer.length / b4096 / b3; i++) {
